...
namespace DotnetAPIProject.Controllers
{
    ...
    public class QuestionPictureController : ControllerBase
    {
        ...
        // post text
        [HttpPost("text")]
        public async Task<IActionResult> PostQuestionText([FromBody] PostQuestionTextRequest request)
        {
        ...
        // post img 
        [HttpPost("image")]
        public async Task<IActionResult> PostQuestionImage([FromBody] PostQuestionImageRequest request)
        {
            if (!ModelState.IsValid)
                return BadRequest(ModelState);
